...
import openai
# OpenAI APIのAPIキーの設定
openai.api_key = "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"
class_names = ['sad', 'happy', 'disgust', 'surprise', 'fear', 'neutral', 'angry']
myclass_names = ['sad', 'happy', 'disgust', 'surprise', 'fear', 'angry']
ans = { 'angry':'私は怒っています','sad': '私は悲しいです' , 'happy': '私は幸せです','disgust': '私は苦しいです','surprise': 'びっくりしました','fear':'私は恐れています','neutral':'私は正常です'}
AI_input ="あなたは、愚痴を聞いてくれる優しい恋人です。相手をひたすら肯定します。励まして勇気づけます。相手の意見を否定しません。アドバイスを言いません"
# システムメッセージをmessagesリストに追加
st.session_state.messages=[{"role": "system", "content":AI_input}]
...
time_threshold = st.sidebar.slider("time threshold",min_value=1,max_value=50,value=5)
# 顔が何フレームつづけて検出されたら顔とみなすか
frame_threshold = st.sidebar.slider("frame threshold",min_value=1,max_value=5,value=5)
...
# if webrtc.video_processor:
    while True:
        ...
        analysis.table(df1)
        emotions.table(df2)
        if objs['dominant_emotion'] in myclass_names:
            et = emotion_text.text_area(label='私',value=ans[objs['dominant_emotion']],key=time.time())
            st.session_state.messages.append({"role": "user", "content": et})
            messages = st.session_state.messages
            response = openai.ChatCompletion.create(model="gpt-3.5-turbo-0301",messages=messages)
            ai_response = response['choices'][0]['message']['content']
            ai_text.text_area(label='ChatGPT',value=ai_response,key=time.time())
